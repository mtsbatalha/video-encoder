...
class RecurrentFolderUI:
    ...
    def run(self):
        ...
        while True:
            self.menu.clear()
            self.menu.print_header("Gerenciador de Pastas Recorrentes", "Monitoramento contínuo de diretórios")
    ...
    def list_recurrent_folders(self):
        ...
        table = Table(title="Pastas Recorrentes", show_header=True, header_style="bold magenta")
        ...
        for i, folder in enumerate(folders, 1):
            ...
            status = "[green]OK[/green]" if input_exists else "[red]Pasta não existe[/red]"
            
            enabled = folder.get("enabled", False)
            enabled_status = "[green]Ativa[/green]" if enabled else "[yellow]Desativada[/yellow]"
    ...
    def add_recurrent_folder(self):
        ...
        preserve_subdirs = self.menu.ask_confirm(
            "Preservar subdiretórios no output?",
            default=True
        )
        ...
        try:
            ...
            self.menu.print_success(f"Pasta recorrente adicionada com sucesso (ID: {folder_id[:8]}...)")
    ...
    def remove_recurrent_folder(self):
        ...
        for i, folder in enumerate(folders, 1):
            enabled_status = "[green]Ativa[/green]" if folder.get("enabled", False) else "[yellow]Desativada[/yellow]"
            table.add_row(
                str(i),
                folder.get("name", "N/A"),
        ...
        self.menu.console.print(f"  Total processado: {folder.get('total_processed', 0)}")
    ...
    def edit_recurrent_folder(self):
        ...
        self.menu.print_info("Perfis disponíveis:")
        for i, p in enumerate(profiles, 1):
            current_marker = " (atual)" if p["id"] == folder.get("profile_id", "") else ""
        ...
        updates = {
            ...
            "options": {
                ...
                "delete_source_after_encode": delete_source,
                "copy_subtitles": copy_subtitles,
                "supported_extensions": options.get("supported_extensions", [".mp4", ".mkv", ".avi", ".mov", ".wmv", ".flv", ".webm", ".m4v", ".mpeg", ".mpg"]),
    ...
    def toggle_enable_folder(self):
        ...
        for i, folder in enumerate(folders, 1):
            enabled = folder.get("enabled", False)
            status = "[green]Ativa[/green]" if enabled else "[yellow]Desativada[/yellow]"
            table.add_row(
                str(i),
                folder.get("name", "N/A"),
        ...
        if not self.menu.ask_confirm(f"Deseja {action} a pasta '{folder_name}'?", default=False):
            self.menu.print_warning("Operação cancelada")
            input("\nPressione Enter para continuar...")
            return
        ...
        if success:
            self.menu.print_success(f"Pasta '{folder_name}' {new_status_text.lower()} com sucesso")
        else:
    ...
    def start_stop_monitors(self):
        ...
        table = Table(title="Status dos Monitores", show_header=True, header_style="bold magenta")
        table.add_column("#", style="dim", width=4)
        table.add_column("Nome", style="cyan")
        table.add_column("Entrada", style="white")
        table.add_column("Status", style="yellow")
        ...
        for i, folder in enumerate(folders, 1):
            enabled = folder.get("enabled", False)
            status = "[green]Ativa[/green]" if enabled else "[yellow]Desativada[/yellow]"
            # Em uma implementação futura, aqui mostraria se o monitor está realmente rodando
            monitor_status = "[green]Pronto[/green]" if enabled else "[dim]Parado[/dim]"
        ...
        if choice == "1":
            ...
            if not enabled_folders:
                ...
                self.menu.print_success(f"Monitores prontos para {len(enabled_folders)} pasta(s) ativa(s)")
                self.menu.console.print("\n[dim]Nota: O serviço de monitoramento será iniciado em segundo plano.[/dim]")
            input("\nPressione Enter para continuar...")
        
        elif choice == "2":
            self.menu.print_warning("Parando todos os monitores...")
            self.menu.console.print("\n[dim]Nota: Os monitores serão parados gracefully.[/dim]")
            input("\nPressione Enter para continuar...")
    ...
    def view_history(self):
        ...
        table = Table(title="Resumo de Processamento por Pasta", show_header=True, header_style="bold magenta")
        table.add_column("#", style="dim", width=4)
        table.add_column("Nome", style="cyan")
        ...
        for i, folder in enumerate(folders, 1):
            ...
            stats = self.history_manager.get_stats(folder_id) if folder_id else {
                'total_processed': 0,
                'success_count': 0,
                'failed_count': 0,
                'average_duration': 0,
        ...
        try:
            choice = Prompt.ask("[bold cyan]Escolha uma pasta (1-{0}) ou 0 para voltar[/bold cyan]".format(len(folders)), default="0")
    ...
    def _view_detailed_history(self, folder: Dict[str, Any]):
        ...
        history = self.history_manager.get_recent_entries(folder_id, limit=50)  # Últimos 50 registros
        ...
        table = Table(title=f"Registros Recentes - {folder.get('name', 'N/A')}", show_header=True, header_style="bold magenta")
        ...
        for entry in history:
            ...
            table.add_row(date_str, status_display, duration_str, input_path, output_path)
        ...
        self.menu.console.print()
        self.menu.print_info("Digite o número da pasta para ver detalhes (0 para voltar)")
