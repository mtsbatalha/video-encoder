    ...
    try:
        ...
        if msvcrt.kbhit():
            ...
            if char.lower() == "d":
                ...
                encoder.realtime_monitor._add_debug_log(
                    f"Debug {'ativado' if debug_enabled else 'desativado'} via tecla D"
                )
                return True
    except ImportError:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config", type=str, help="Caminho para arquivo de config personalizado"
    )

    group_mode = parser.add_mutually_exclusive_group()
    group_mode.add_argument(
        "--watch", action="store_true", help="Modo watch (monitorar pastas)"
    )
    group_mode.add_argument("-f", "--file", type=str, help="Codificar arquivo único")
    group_mode.add_argument(
        "-F", "--folder", type=str, help="Codificar todos vídeos de uma pasta"
    )
    group_mode.add_argument(
    ...
    parser.add_argument(
        "-p", "--profile", type=str, help="Perfil de encoding (nome ou ID)"
    )
    parser.add_argument("-o", "--output", type=str, help="Diretório de output")
    parser.add_argument(
        "--output-file",
        type=str,
        help="Arquivo de output específico (para arquivo único)",
    )

    parser.add_argument(
        "--codec",
        type=str,
        choices=["hevc_nvenc", "h264_nvenc", "av1_nvenc"],
        help="Codec de vídeo",
    )
    parser.add_argument(
        "--cq", type=str, help="Constant Quality (1-51 para HEVC/H264, 1-63 para AV1)"
    )
    ...
    parser.add_argument(
        ...
        help="Preset de encoding",
    )
    parser.add_argument(
        "--resolution",
        type=str,
        choices=["480", "720", "1080", "1440", "2160"],
        help="Resolução de output",
    )
    parser.add_argument(
        "--two-pass", action="store_true", help="Usar two-pass encoding"
    )
    parser.add_argument(
        "--hdr-to-sdr", action="store_true", help="Converter HDR para SDR"
    )
    parser.add_argument(
        "--deinterlace", action="store_true", help="Aplicar deinterlacing"
    )

    parser.add_argument(
        "--profile-list", action="store_true", help="Listar perfis disponíveis"
    )
    parser.add_argument(
        "--profile-create", type=str, metavar="NAME", help="Criar novo perfil"
    )
    parser.add_argument(
        "--profile-export",
        type=str,
        metavar="PROFILE",
        help="Exportar perfil para JSON",
    )
    parser.add_argument(
        "--profile-import", type=str, metavar="FILE", help="Importar perfil de JSON"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="FILE",
        help="Exportar estatísticas para JSON",
    )
    parser.add_argument(
        "--stats-reset", action="store_true", help="Resetar estatísticas"
    )
    ...
    parser.add_argument(
        "--check", action="store_true", help="Verificar instalação (FFmpeg, GPU)"
    )

    parser.add_argument(
        "--detect-hardware",
        action="store_true",
        help="Detectar hardware e mostrar codecs disponíveis",
    )
    parser.add_argument(
        "--hardware",
        type=str,
        choices=["nvidia_gpu", "amd_gpu", "intel_igpu", "amd_igpu", "cpu"],
        help="Filtrar perfis por categoria de hardware",
    )
...
def cmd_check(args, config: ConfigManager):
    ...
    if ffmpeg.verify_installation():
        ...
        if codecs:
            console.print(
                f"[green][OK][/green] Codecs NVENC disponíveis: {', '.join(codecs)}"
            )
        else:
    ...
    if importlib.util.find_spec("psutil") is not None:
        ...
        console.print(
            "[yellow][!][/yellow] psutil não instalado (monitoramento CPU limitado)"
        )
    ...
    if stats.gpu_util >= 0 or stats.gpu_temperature > 0:
        ...
        console.print(
            "[yellow][!][/yellow] GPU não detectada ou nvidia-smi não disponível"
        )
...
def cmd_detect_hardware(args, profile_mgr: ProfileManager):
    ...
    if hw_info.get("gpus_nvidia"):
        for gpu in hw_info["gpus_nvidia"]:
            console.print(
                f"  [green]•[/green] {gpu.get('name', 'NVIDIA GPU')} ({gpu.get('memory_gb', 0)}GB VRAM)"
            )
            console.print(
                f"      NVENC: {'[green]Suportado[/green]' if gpu.get('nvenc_supported') else '[red]Não suportado[/red]'}"
            )
    else:
    ...
    if hw_info.get("gpus_amd"):
        for gpu in hw_info["gpus_amd"]:
            ...
            console.print(
                f"      AMF: {'[green]Suportado[/green]' if gpu.get('amf_supported') else '[red]Não suportado[/red]'}"
            )
    else:
    ...
    if hw_info.get("igpu_intel"):
        ...
        console.print(
            f"      QSV: {'[green]Suportado[/green]' if igpu.get('qsv_supported') else '[red]Não suportado[/red]'}"
        )
    else:
    ...
    if hw_info.get("igpu_amd"):
        ...
        console.print(
            f"      AMF: {'[green]Suportado[/green]' if igpu.get('amf_supported') else '[red]Não suportado[/red]'}"
        )
    else:
...
def cmd_profile_create(args, profile_mgr: ProfileManager, menu: Menu):
    ...
    console.print(
        f"[green][OK][/green] Perfil criado com ID: [cyan]{profile_id}[/cyan]"
    )
...
def cmd_profile_export(args, profile_mgr: ProfileManager):
    ...
    profile = profile_mgr.get_profile(
        args.profile_export
    ) or profile_mgr.get_profile_by_name(args.profile_export)
...
def cmd_stats(args, stats_mgr: StatsManager):
    ...
    if args.stats_export:
        if stats_mgr.export_to_json(args.stats_export):
            console.print(
                f"[green][OK][/green] Estatísticas exportadas para: {args.stats_export}"
            )
        else:
...
def run_watch_mode(
    ...
    encoder = EncoderEngine(
        max_concurrent=config.get("encoding.max_concurrent", 2), queue_manager=queue_mgr
    )
...
def run_single_file(
    ...
    if args.profile:
        profile = profile_mgr.get_profile(
            args.profile
        ) or profile_mgr.get_profile_by_name(args.profile)
        if not profile:
    ...
    if args.output_file:
        ...
        output_path = PathUtils.generate_output_path(
            input_path, args.output, codec=codec, cq=cq, handle_conflict=False
        )
    else:
        output_path = PathUtils.generate_output_path(
            input_path,
    ...
    if Path(output_path).exists():
        ...
        # Opção 1: Substituir
        if Confirm.ask(
            "\nDeseja [bold red]substituir[/bold red] o arquivo existente?",
            default=False,
        ):
            strategy = FileConflictStrategy.OVERWRITE
        # Opção 2: Renomear com numeração
        elif Confirm.ask(
            "Deseja [bold green]criar uma cópia numerada[/bold green] (ex: arquivo_1.mkv)?",
            default=True,
        ):
    ...
    if success:
        ...
        job_mgr.update_job_status(
            job_id, job_status, input_size=input_size, output_size=output_size
        )
...
def run_folder_mode(
    ...
    if not profile:
        console.print(
            "[red][X][/red] Perfil necessário para modo folder. Use -p ou --profile"
        )
        return
    ...
    is_valid, msg, suggested_name = FileUtils.validate_output_folder_name(
        str(output_path_obj), codec=codec, quality=profile.get("resolution")
    )
    if not is_valid and suggested_name:
        console.print(f"[yellow][!][/yellow] {msg}")
        if Confirm.ask(
            f"Deseja usar o nome sugerido: [cyan]{suggested_name}[/cyan]?", default=True
        ):
    ...
    for video_file in video_files:
        ...
        if str(rel_parent) != ".":
            ...
            base_folder_name = PathUtils.generate_output_dir_name(
                str(rel_parent), codec, cq
            )
            ...
            is_valid, msg, suggested = FileUtils.validate_output_folder_name(
                str(temp_folder_path), codec=codec, quality=profile.get("resolution")
            )
        ...
        output_path = PathUtils.generate_output_path(
            video_file, video_output_dir, codec=codec, cq=cq, handle_conflict=False
        )
        ...
        FileUtils.copy_subtitles_to_output(
            source_dir, video_output_dir, Path(video_file).stem
        )
...
def run_folder_conversion_cli(
    ...
    from src.managers.multi_profile_conversion_manager import (
        MultiProfileConversionManager,
        NamingConvention,
    )
    from src.ui.remote_connection_ui import RemoteConnectionUI
    from src.managers.remote_directory_manager import (
    ...
    if directory_type == "remote":
        ...
        if not Confirm.ask(
            ...
            success, msg = remote_manager.test_connection(
                remote_path, connection_config
            )
            if not success:
        ...
        menu.print_info(
            "Copiando arquivos do diretório remoto para diretório temporário..."
        )
        ...
        success, temp_dir, files = remote_manager.copy_directory_to_temp(
            remote_path, connection_config, progress_callback=on_progress
        )
    ...
    if mode_choice == 0:
        ...
        profile_idx = menu.show_options(
            [p["name"] for p in profiles], "Perfis disponíveis"
        )
        ...
        selected_profile_ids = menu.show_multi_profile_selection(
            profiles, "Selecione os Perfis para Conversão"
        )
    ...
    console.print(
        f"[green]Encontrados {len(video_files)} arquivo(s) de vídeo[/green]\n"
    )
    ...
    if len(selected_profile_ids) > 1:
        multi_mgr = MultiProfileConversionManager(
            profile_manager=profile_mgr, job_manager=job_mgr, queue_manager=queue_mgr
        )
        ...
        while True:
            ...
            if preview_action == 0:  # Confirmar
                ...
                console.print(
                    f"[cyan]Fila total: {queue_mgr.get_queue_length()} jobs[/cyan]"
                )
                ...
                console.print(
                    f"[cyan]Legendas copiadas para os diretórios de saída[/cyan]"
                )
                ...
                selected_profile_ids = menu.show_multi_profile_selection(
                    profile_mgr.list_profiles(), "Selecione os Perfis para Conversão"
                )
                ...
                edited_profiles = multi_mgr.edit_individual_profiles_interactive(
                    selected_profile_ids, profile_mgr
                )
    ...
    while True:
        ...
        if action_choice == 0:  # 🚀 Iniciar conversão agora
            ...
            for video_file in video_files:
                ...
                if str(rel_parent) != ".":
                    folder_name = PathUtils.generate_output_dir_name(
                        str(rel_parent), codec, cq
                    )
                ...
                ensure_directory(video_output_dir)
                output_path = PathUtils.generate_output_path(
                    video_file, video_output_dir, codec=codec, cq=cq
                )
            ...
            menu.print_success(
                f"{len(video_files)} job(s) adicionados à fila e processamento iniciado"
            )
            ...
            if (
                ...
                and temp_dir
                and remote_manager.should_auto_cleanup()
            ):
            ...
            console.print(
                "[dim]Use a opção 'Ver fila de jobs' no menu principal para gerenciar[/dim]"
            )
            ...
            if profile.get("id") == "manual":
                profile["name"] = (
                    f"Manual ({codec} CQ{cq or profile.get('bitrate', 'auto')})"
                )
...
def run_folder_conversion_submenu(
    config: ConfigManager,
    profile_mgr: ProfileManager,
    job_mgr: JobManager,
    queue_mgr: QueueManager,
    ...
    while True:
        ...
        if choice == 0:
            run_single_folder_conversion(
                config, profile_mgr, job_mgr, queue_mgr, stats_mgr
            )
        elif choice == 1:
            run_recurrent_folder_from_submenu(config, profile_mgr, job_mgr, queue_mgr)
        elif choice == 2:
            break


def run_single_folder_conversion(
    config: ConfigManager,
    profile_mgr: ProfileManager,
    job_mgr: JobManager,
    queue_mgr: QueueManager,
...
def run_recurrent_folder_from_submenu(
    config: ConfigManager,
    profile_mgr: ProfileManager,
    ...
    recurrent_ui = RecurrentFolderUI(
        console, config, profile_mgr, queue_manager=queue_mgr, job_manager=job_mgr
    )
    recurrent_ui.run()
...
def validate_directory_exists(
    path: str, create_if_not: bool = False
) -> tuple[bool, str]:
...
def run_single_file_cli(
    ...
    cq = profile.get("cq")
    output_path = PathUtils.generate_output_path(
        input_path, output_dir, codec=codec, cq=cq
    )
...
def run_folder_mode_cli(
    ...
    output_dir = menu.ask(
        "Diretório de output", default=str(Path(folder_path) / "converted")
    )
    ensure_directory(output_dir)
