...
class WatchFolderMonitor:
    ...
    def __init__(self, config: Dict[str, Any], queue_manager: QueueManager, job_manager: JobManager, profile_manager: ProfileManager, history_manager: Optional[RecurrentHistoryManager] = None):
        self.config = config
        ...
        self.folder_id = config.get('folder_id', '')  # ID da pasta recorrente (opcional)
    ...
    def start(self) -> None:
        ...
        if self._monitor_thread and self._monitor_thread.is_alive():
            self.logger.warning(f"Monitoramento já está em execução para {self.watch_path}")
            return
                ...
                try:
                    if self._should_process_file(file_path):
                        if self._is_file_complete(file_path):
                            ...
                            self.logger.debug(f"Arquivo ainda sendo copiado, aguardando: {file_path}")
                except Exception as e:
    ...
    def _is_file_complete(self, path: Path) -> bool:
        ...
        try:
            ...
            if initial_size != current_size:
                self.logger.debug(f"Tamanho do arquivo mudou ({initial_size} -> {current_size}), ainda sendo copiado: {path}")
                return False
        ...
        try:
            ...
            try:
                ...
                if file_size < self.min_size:
                    self.logger.debug(f"Arquivo abaixo do tamanho mínimo ({self.min_size} bytes): {path} ({file_size} bytes)")
                    return False
            except OSError:
            ...
            self.logger.error(f"Erro ao verificar se deve processar arquivo {path}: {e}")
            return False
    ...
    def _get_output_path(self, input_path: Path) -> Optional[Path]:
        ...
        try:
            ...
            output_dir_str = self.config.get('output_path', str(input_path.parent / 'encoded'))
    ...
    def _enqueue_file(self, path: Path) -> None:
        ...
        try:
            ...
            if self.history_manager and self.folder_id:
                self.job_manager.register_status_callback(job_id, self._on_job_status_change)
            ...
            self.logger.info(f"Arquivo adicionado à fila: {path} -> {output_path} (posição: {queue_pos})")
            
        except Exception as e:
            self.logger.error(f"Erro ao enfileirar arquivo {path}: {e}")
    
    def _on_job_status_change(self, job_id: str, old_status: str, new_status: str) -> None:
            ...
            if new_status in ['completed', 'failed']:
                ...
                if started_at_str and completed_at_str:
                    try:
                        ...
                        error_message = job.get('error_message') if new_status == 'failed' else None
                        ...
                        self.logger.info(f"Registro de histórico adicionado para job {job_id} (status: {new_status})")
                    except Exception as e:
                        self.logger.error(f"Erro ao converter timestamps para job {job_id}: {e}")
        except Exception as e:
