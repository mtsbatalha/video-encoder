...
class RecurrentMonitorService:
            ...
            if folder_config.get('enabled', True):
                try:
                    ...
                    if monitor:
                        ...
                        self.logger.error(f"Falha ao criar monitor para a pasta: {folder_id}")
                except Exception as e:
                    self.logger.error(f"Erro ao iniciar monitor para a pasta {folder_id}: {str(e)}")
        
        self.logger.info(f"{len(started_monitors)} monitores iniciados com sucesso.")
        return started_monitors
    
    def _create_monitor(self, folder_id: str, folder_config: Dict[str, Any]) -> Optional[WatchFolderMonitor]:
        """
        ...
        try:
            ...
            watch_config = {
                ...
                'output_path': folder_config.get('output_directory'),  # Caminho de saída
                'profile_id': folder_config.get('profile_id'),  # ID do perfil
                'folder_id': folder_id,  # ID da pasta recorrente
                'interval': folder_config.get('interval', 10),  # Intervalo de verificação
                'min_size': folder_config.get('min_size', 10 * 1024 * 1024),  # Tamanho mínimo
                'skip_existing_output': folder_config.get('skip_existing_output', True),
                'extensions': folder_config.get('supported_extensions', ['.mp4', '.mkv', '.avi', '.mov', '.wmv', '.flv', '.webm']),
            ...
            self.logger.error(f"Erro ao criar monitor para a pasta {folder_id}: {str(e)}")
            return None
    ...
    def stop_all_monitors(self) -> None:
        ...
        for folder_id, monitor in self.monitors.items():
            try:
                ...
                self.logger.error(f"Erro ao parar monitor para a pasta {folder_id}: {str(e)}")
    ...
    def stop_monitor(self, folder_id: str) -> bool:
        ...
        if folder_id not in self.monitors:
            self.logger.warning(f"Nenhum monitor ativo encontrado para a pasta: {folder_id}")
            return False
